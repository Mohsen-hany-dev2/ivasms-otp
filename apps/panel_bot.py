...
class PanelBot:
    ...
    def collect_managed_bots_groups(self) -> list[dict[str, str]]:
        ...
        for row in rows:
            token = str(row.get("token") or row.get("bot_token") or "").strip()
            if not token:
                continue
            storage = str(row.get("storage") or row.get("storage_mode") or "private").strip().lower()
            ...
            if storage == "shared":
                ...
                groups = []
                if bot_id:
                    db_path = BASE_DIR / "data" / f"bot_{bot_id}" / "storage.db"
                    if db_path.exists():
                        try:
                            conn = sqlite3.connect(db_path)
                            cur = conn.cursor()
                            row_db = cur.execute("SELECT value FROM kv_store WHERE key='groups'").fetchone()
                            conn.close()
                            if row_db:
                                payload = json.loads(str(row_db[0]))
                                if isinstance(payload, list):
                                    groups = [x for x in payload if isinstance(x, dict)]
                        except Exception:
                            groups = []
            for g in groups:
                if not bool(g.get("enabled", True)):
                    continue
                chat_id = str(g.get("chat_id") or "").strip()
                if not chat_id:
                    continue
                key = (token, chat_id)
                if key in seen:
                    continue
                seen.add(key)
                out.append({"token": token, "chat_id": chat_id, "name": str(g.get("name") or chat_id)})
        return out

    def send_with_token(self, token: str, method: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    def run_broadcast(self, user_id: int, chat_id: int, text: str, button_text: str = "", button_url: str = "") -> None:
        targets = self.collect_managed_bots_groups()
        kb = None
        ...
        for target in targets:
            ...
            res = self.send_with_token(target["token"], "sendMessage", body)
    ...
    def handle_callback(self, q: dict[str, Any]) -> None:
        ...
        if data == "settings_broadcast":
            if not self.has_full_vars_access(user_id):
                ...
                return
            self.set_state(user_id, "wait_broadcast_text")
            self.send_text(
                chat_id,
                self._tr(
                    user_id,
                    "اكتب نص الإذاعة الآن.",
                    "Send broadcast text now.",
                ),
            )
            return

        if data.startswith("broadcast_has_btn:"):
            if not self.has_full_vars_access(user_id):
                self.answer_callback(callback_id, self._tr(user_id, "غير متاح.", "Not allowed."))
                return
            choice = data.split(":", 1)[1].strip().lower()
            st = self.get_state(user_id) or {}
            if st.get("mode") != "wait_broadcast_has_button":
                self.send_text(chat_id, self._tr(user_id, "ابدأ الإذاعة من الإعدادات.", "Start broadcast from settings."))
                return
            payload = st.get("data") or {}
            msg_text = str(payload.get("text") or "").strip()
            if not msg_text:
                self.clear_state(user_id)
                self.send_text(chat_id, self._tr(user_id, "نص الإذاعة مفقود.", "Broadcast text missing."))
                return
            if choice == "yes":
                self.set_state(user_id, "wait_broadcast_button_text", {"text": msg_text})
                self.send_text(chat_id, self._tr(user_id, "اكتب نص الزر.", "Send button text."))
                return
            self.clear_state(user_id)
    ...
    def handle_text_message(self, msg: dict[str, Any]) -> None:
        ...
        if mode == "wait_broadcast_text":
            payload = {"text": text.strip()}
            self.set_state(user_id, "wait_broadcast_has_button", payload)
            kb = self._pattern_rows(
                [
                    self._btn(self._tr(user_id, "✅ نعم", "✅ Yes"), callback_data="broadcast_has_btn:yes", style="success"),
                    self._btn(self._tr(user_id, "❌ لا", "❌ No"), callback_data="broadcast_has_btn:no", style="danger"),
                ],
                back_callback="vars_menu",
                back_text=self._tr(user_id, "رجوع", "Back"),
            )
            self.send_text(chat_id, self._tr(user_id, "هل تريد إضافة زر للرسايل؟", "Do you want a button in broadcast?"), kb)
            return

        if mode == "wait_broadcast_button_text":
            payload = dict(data)
            payload["button_text"] = text.strip()
            self.set_state(user_id, "wait_broadcast_button_url", payload)
            self.send_text(chat_id, self._tr(user_id, "اكتب رابط الزر.", "Send button URL."))
            return

        if mode == "wait_broadcast_button_url":
            payload = dict(data)
            url = str(text).strip()
            if not (url.startswith("http://") or url.startswith("https://") or url.startswith("t.me/") or url.startswith("https://t.me/")):
                self.send_text(chat_id, self._tr(user_id, "رابط غير صحيح.", "Invalid URL."))
                return
            if url.startswith("t.me/"):
                url = "https://" + url
            b_text = str(payload.get("button_text") or "").strip()
            msg_text = str(payload.get("text") or "").strip()
            self.clear_state(user_id)
            self.send_text(chat_id, self._tr(user_id, "جاري الإذاعة...", "Broadcast in progress..."))
